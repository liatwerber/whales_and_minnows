class ReviewsController < ApplicationController
  before_action :current_user_must_be_review_commenter, only: [:edit, :update, :destroy] 
  ...
  def index
    ...
    @reviews = @q.result(:distinct => true).includes(:commenter, :photo).page(params[:page]).per(10)
  end
  ...
  def current_user_must_be_review_commenter
    ...
    def review_params
      params.require(:review).permit(:body, :reviewer_id, :course_id, :course_rating)
    end
end
